...
class MediaPartner {
  ...
  public static getAll = async (): Promise<MediaPartner[]> => {
    const locale = i18next.language;
    const response = await fetch(
      import.meta.env.PUBLIC_CMS_API_ROUTE +
        "/content/items/mediaPartners?locale=" +
        locale,
      {
        method: "GET",
        headers: {
    ...
    return data.map((item: MediaPartner) => new MediaPartner(item));
  };
