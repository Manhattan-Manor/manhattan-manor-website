...
class PreferredVendor {
  ...
  constructor(item?: PreferredVendor) {
    if (item) {
  ...
  public static getAll = async (): Promise<PreferredVendor[]> => {
    const locale = i18next.language;
    const response = await fetch(
      import.meta.env.PUBLIC_CMS_API_ROUTE +
        "/content/items/preferredVendors?locale=" +
        locale,
      {
        method: "GET",
        headers: {
    ...
    return data.map((item: PreferredVendor) => new PreferredVendor(item));
  };
